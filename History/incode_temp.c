...
void repeat_characters(unsigned char *buff) {	//BBCBBCBBCBBC -> BBC*3
	unsigned char *temp;
...
int main(int argc, char* argv[]) {
	...
	checkout_buff(last_buf, des_buff);	//전주성: 약속대로 Description 압축하는 함수
	checkout_same_line(last_buf);				//전주성: 압축된 문자열에서 같은 줄은 =1 로 치환하는 함수
	repeat_characters(last_buff);
	insert_file(last_buf);							//전주성: 파일에 써넣는 함수
