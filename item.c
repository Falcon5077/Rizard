...
// 함수 선언
void Check_Item(char*);
char Check_Item_sort(char*);
void Write_Item(char*, char);
...
void Check_Item(char* tmp_Items) {
    char buf[1000];
...
char Check_Item_sort(char* tmp_Items) {
    int i, num;
    char tmp_count[6] = {0}; // ptr과 Items[i]와 같을때의 i값
    int for_count = 0; // for문 돌아가는 횟수
    char sort = 0;

    str = strtok(tmp_Items, "/"); // 슬래쉬(/) 단위로 잘라서 str에 저장
...
void Write_Item(char* tmp_Items, char sort) {
    int num, tmp_count = 0;
